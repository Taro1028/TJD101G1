...
export const useMemberStore = defineStore("member", {
  ...
  getters: {
    ...
    userAvatar: (state) => {
      ...
      // 最後使用預設頭像
      return new URL('../assets/images/Member/person-fill.svg', import.meta.url).href;
    },
  ...
  actions: {
    ...
    setMember(member) {
      ...
      if (member.success) {
        ...
        console.log(`會員 ${this.name} 登入成功，頭像狀態:`, this.avatar ? '自訂頭像' : '預設頭像');
      }
    },
    ...
    updateAvatar(avatarData) {
      ...
      // 這裡之後可以加入 API 呼叫，將頭像上傳到伺服器
      // 例如：await this.uploadAvatarToServer(avatarData);
    },
    ...
    saveMemberAvatars() {
      try {
        localStorage.setItem('memberAvatars', JSON.stringify(this.memberAvatars));
      } catch (error) {
    ...
    saveToLocalStorage() {
      try {
        ...
        console.error('儲存會員資料到 localStorage 失敗:', error);
      }
    },
