...
export const useCheckoutStore = defineStore('checkout', () => {
  ...
  const selectedTotalAmount = computed(() => {
    return selectedCartItems.value.reduce((sum, group) => sum + (group.total_amount || 0), 0)
  })
  
  // 計算被選中項目的總餐數
  const selectedTotalMeals = computed(() => {
    return selectedCartItems.value.reduce((sum, group) => sum + (group.total_meal_count || 0), 0)
  })
