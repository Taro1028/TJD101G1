    ...
    <div class="wrapper">
      ...
      <div v-else class="infoContainer">
        ...
        <div class="orderData">
          ...
          <div class="orderlist">
            <!-- 訂單項目 -->
            <div v-for="group in cartGroups" :key="group.cart_id" class="orderItem">
              <!-- 項目區塊(項目:包含標題資料 + 垃圾桶) -->
              <div class="item">
                <!-- 項目資料(標題、期間 & 展開按鈕) -->
                <div class="itemMeta">
                  <label class="checkedItem">
                    <input type="checkbox" :checked="selectedItems.has(group.cart_id)"
                  ...
                  <div class="period_toggle">
                    ...
                    <button class="btn_accordion" @click="toggleDetails(group.cart_id)">
                      <i :class="showAllDetails.has(group.cart_id) ? 'bi bi-chevron-up' : 'bi bi-chevron-down'"></i>
                    </button>
                  </div>
                </div>
                <button class="btn_delete" @click="removeCartItem(group.cart_id, group.plan_type)">
              ...
              <div v-if="showAllDetails.has(group.cart_id)" class="content_container">
                ...
                <div class="itemMetadata">
                  ...
                  <!-- 標身 - 每日餐點詳情 -->
                  <div v-for="item in group.items" :key="`${group.cart_id}-${item.date}`" class="detailItem">
                    <div class="day_box">
        ...
        <div class="payData">
          ...
          <div class="btnblock">
            <button class="btn-2" @click="goNext" :disabled="!canProceed" :class="{ disabled: !canProceed }">
              下一步
            </button>
          </div>
        </div>
